...
def plot_results_functions(apx_drift_diffusivity, true_drift_diffusivity,
    ...
    if n_dimensions == 1:
        ...
        if fig is None:
            fig, ax = plt.subplots(2 * rows_per_function, figures_per_row, figsize=(n_dimensions * 2, 6 * rows_per_function))
...
def data_transform(x):
    if x.min() == x.max():
        ...
        # Otherwise, scale the output of the SDE to the range 0 to 1
...
def plot_results_1d(time_steps, paths_network, paths_true, plot_dim=0, linewidth=0.5, figsize=(8, 6)):
    ...
    for k in range(len(paths_network)):
        ...
        ax.plot(time_steps*25, path_network[:, plot_dim], 'r-', linewidth=linewidth,
                label='Paths with SDE Model' if k == 0 else None)
        ax.plot(time_steps*25, path_true[:, plot_dim], 'k-', linewidth=linewidth,
                label='Paths with Test Simulation' if k == 0 else None)

    ax.set_xlabel("Time")
    ax.set_ylabel("Infected Rate (starting 0.01)")

    ax.legend();
    fig.tight_layout()

    return fig
def plot_results_1d_generated(time_steps, paths_network, plot_dim=0, linewidth=0.5, figsize=(8, 6)):
    ...
    for k in range(len(paths_network)):
        ...
        ax2.plot(time_steps*25, path_network[:, plot_dim], 'black', linewidth=linewidth,
                label='Paths Generated with SDE Model' if k == 0 else None)
    ...
    ax2.legend();
    ax2.set_xlabel("Time")
    ax2.set_ylabel("Infected Rate (starting 0.01)")
    fig2.tight_layout()

    return fig2
def plot_distribution(data,xlabel,ylabel,title, figsize=(8, 6),color='r'):

    # Set the figure size
    fig, ax = plt.subplots(figsize=figsize)

    # Plot the histogram
    ax.hist(data, bins=30, alpha=0.9, color=color)
    ...
    return None
# def plot_results_1d(time_steps, paths_network, plot_dim=0, linewidth=1, figsize=(6, 4)):
#     fig, ax = plt.subplots(1, 1, figsize=figsize)
